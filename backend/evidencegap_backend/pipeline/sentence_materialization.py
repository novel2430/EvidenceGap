...
from evidencegap_backend.pipeline.contracts import (
...
class StanzaSentenceSplitter:
    ...
    def split_many(self, texts: Sequence[str]) -> list[list[SentenceSpan]]:
        if self._pipeline is None:
            raise EvidenceGapError("Stanza sentence splitter is closed")
        if not texts:
            return []
        try:
...
def materialize_runtime_sentences(
    ...
    *,
    ...
    runtime_articles: Sequence[Mapping[str, Any]] | None = None,
) -> dict[str, Any]:
    root = root.resolve()
    input_path = input_path.resolve()
    if runtime_articles is None:
        articles = load_runtime_articles(input_path)
        source_handoff = "artifact_reload"
    else:
        articles = [RuntimeArticle.from_mapping(row) for row in runtime_articles]
        if not articles:
            raise EvidenceGapError("In-memory runtime article input cannot be empty")
        article_ids = [article.article_id for article in articles]
        if len(article_ids) != len(set(article_ids)):
            raise EvidenceGapError("Duplicate in-memory runtime article_id")
    ...
    return {
        ...
        "validation": validation,
        "outputs": output_manifest,
        "runtime_article_rows": article_rows,
        "runtime_sentence_rows": sentence_rows,
    }
