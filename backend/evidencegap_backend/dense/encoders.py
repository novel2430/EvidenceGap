...
from evidencegap_backend.common import EvidenceGapError, sha256_file
...
class DenseEncoder:
    ...
    @property
    def query_model_loaded(self) -> bool:
        return self._query_bundle is not None

    def load_query_model(self) -> None:
        """Eagerly load the query tokenizer/model for service startup."""
        self._load(self.spec.query_model, article=False)

    def close(self) -> None:
        """Release loaded model bundles and CUDA cache deterministically."""
        bundles: list[tuple[Any, Any]] = []
        for attribute in ("_query_bundle", "_article_bundle"):
            bundle = getattr(self, attribute)
            if bundle is not None and all(bundle is not existing for existing in bundles):
                bundles.append(bundle)
            setattr(self, attribute, None)
        for _tokenizer, model in bundles:
            try:
                model.to("cpu")
            except Exception:
                pass
