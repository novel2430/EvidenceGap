...
from evidencegap_backend.pipeline.retrieval_adapters import _read_parquet
...
def build_article_prompt_inputs(
    *,
    claim_id: str,
    claim_text: str,
    articles: Sequence[Mapping[str, Any]],
    runtime_sentences: Sequence[Mapping[str, Any]],
) -> list[ArticlePromptInput]:
    normalized_claim_id = claim_id.strip()
    normalized_claim_text = claim_text.strip()
    if not normalized_claim_id or not normalized_claim_text:
        raise EvidenceGapError("claim_id and claim_text are required")
    ordered_articles = sorted(
        [dict(row) for row in articles],
        key=lambda row: int(row["final_article_rank"]),
    )
    if not ordered_articles:
        raise EvidenceGapError("Top article input cannot be empty")
    article_ids = [str(row["article_id"]) for row in ordered_articles]
    if len(article_ids) != len(set(article_ids)):
        raise EvidenceGapError("Top article input contains duplicate article IDs")
    by_article: dict[str, list[Mapping[str, Any]]] = {value: [] for value in article_ids}
    for row in runtime_sentences:
...
def load_article_prompt_inputs(
    ...
    inputs = build_article_prompt_inputs(
        claim_id=claim_id,
        claim_text=claim_text,
        articles=_read_parquet(paths["top_articles"]),
        runtime_sentences=_read_parquet(paths["runtime_sentences"]),
    )
...
def validate_response_payload(
    ...
    *,
    max_evidence_sentences: int = MAX_EVIDENCE_SENTENCES,
) -> list[dict[str, Any]]:
...
def _read_cache(
    ...
    *,
    ...
    if not isinstance(payload, Mapping) or not isinstance(response, Mapping):
        return None
    results = validate_response_payload(
        payload,
        items,
        max_evidence_sentences=max_evidence_sentences,
    )
...
def validate_article_evidence_rows(
    ...
    *,
    source_inputs: Sequence[ArticlePromptInput],
    max_evidence_sentences: int = MAX_EVIDENCE_SENTENCES,
) -> dict[str, Any]:
...
def run_article_evidence_extractor(
    ...
    *,
    ...
    prompt_request: Mapping[str, str] | None = None,
    prompt_items: Sequence[ArticlePromptInput] | None = None,
    ...
    if max_evidence_sentences <= 0:
        raise EvidenceGapError("max_evidence_sentences must be positive")
    prompt = resolve_builtin_prompt(
        prompt_name="article_evidence.txt",
        default_version=ARTICLE_EVIDENCE_PROMPT_VERSION,
    ...
    if (prompt_request is None) != (prompt_items is None):
        raise EvidenceGapError(
            "prompt_request and prompt_items must be provided together"
        )
    if prompt_request is None or prompt_items is None:
        request, items, source_paths = load_article_prompt_inputs(
            retrieval_artifact_dir
        )
        source_mode = "artifact_reload"
    else:
        request = {
            "claim_id": str(prompt_request.get("claim_id") or "").strip(),
            "claim_text": str(prompt_request.get("claim_text") or "").strip(),
        }
        if not request["claim_id"] or not request["claim_text"]:
            raise EvidenceGapError("In-memory prompt request is incomplete")
        items = list(prompt_items)
        if not items:
            raise EvidenceGapError("In-memory prompt items cannot be empty")
        source_mode = "in_memory_handoff"
