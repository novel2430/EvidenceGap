...
ARTICLE_EVIDENCE_PROMPT_VERSION = "phase07_article_evidence_v2"
...
Judge each supplied article only against the CLAIM. Do not use outside knowledge.

Labels:
- support: the article reports evidence that directly increases confidence that the claim is true.
- refute: the article reports evidence that directly increases confidence that the claim is false or materially contradicts it.
- insufficient: the article is related but does not report enough direct evidence for either direction, or its findings are mixed/ambiguous for the exact claim.
...
- Prefer reported results and conclusions over objectives, background, or methods.
- Do not select headings, isolated fragments, incomplete sentences, or the title.
- support/refute must select at least one sentence.
- insufficient must return an empty evidence_sentence_ids list.
- Preserve population, intervention/exposure, comparator, outcome, direction, and scope.
- Check claim applicability before choosing support or refute. The article must directly study the same kind of question as the claim.
- A treatment study in patients who already have the disease does not support or refute a prevention claim unless it directly reports infection incidence or another preventive outcome. Otherwise label it insufficient.
- Disease severity, symptom duration, respiratory support, hospitalization, or mortality outcomes do not by themselves answer a claim about preventing infection.
- A comparison between two active doses does not establish whether supplementation is better than no supplementation unless the reported result directly answers the claim.
- Do not convert association into causation.
- Probabilities must sum to 1 and label must be the highest-probability class.
- Rationale must be one concise English sentence grounded in the article.
- For a general claim that supplementation is effective, a comparison between two active doses or regimens is insufficient. It can only support or refute a claim specifically comparing those doses. Do not infer effectiveness versus no supplementation unless the article contains a placebo, no-supplement, or otherwise valid untreated comparator.

Return JSON only, with exactly one result for each article_id and no extra items."""
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            return validate_response_payload(response.payload, items), response, attempt
