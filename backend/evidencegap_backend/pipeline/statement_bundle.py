...
def run_statement_bundle(
    ...
    *,
    ...
    statement_result: Mapping[str, Any] | None = None,
    graphs_by_claim: Mapping[str, Mapping[str, Any]] | None = None,
    force: bool = False,
) -> dict[str, Any]:
    root = root.resolve()
    ...
    supplied = (decomposition, statement_result, graphs_by_claim)
    if any(value is None for value in supplied) and not all(
        value is None for value in supplied
    ):
        raise EvidenceGapError(
            "decomposition, statement_result, and graphs_by_claim must be provided together"
        )
    if all(value is None for value in supplied):
        validate_statement_analysis_artifact(analysis_dir)
        decomposition_value = _read_json_object(decomposition_path)
        statement_result_value = _read_json_object(statement_result_path)
        graphs: dict[str, Mapping[str, Any]] = {}
        for result in statement_result_value.get("claim_results", []):
        ...
        decomposition_value = dict(decomposition or {})
        statement_result_value = dict(statement_result or {})
        graphs = {
            str(key): dict(value)
            for key, value in (graphs_by_claim or {}).items()
        }
    ...
    bundle = build_statement_bundle(
        decomposition_value, statement_result_value, graphs
    )
